...
public class MedianOfTwoSortedArrays {
    ...
    IntStream.range(0, nums1.length)
        .forEachOrdered(
            i -> {
              System.out.println(
                  "Input : nums1 -> "
                      + Arrays.toString(nums1[i])
                      + " | nums2 -> "
                      + Arrays.toString(nums2[i]));
              System.out.println(
                  "Output : " + medianOfTwoSortedArrays.findMedianSortedArrays(nums1[i], nums2[i]));
  ...
  public double findMedianSortedArrays(int[] nums1, int[] nums2) {
    ...
    System.out.println(
        "array after modification " + Arrays.toString(nums1) + " : " + Arrays.toString(nums2));
    if (totalLen % 2 == 0) {
      int greaterIndex = totalLen / 2;
      int smallerIndex = greaterIndex - 1;
      if (smallerIndex >= nums1.length) {
        return ((nums2[remainderWithCheck(smallerIndex, nums1.length)]
                    + nums2[remainderWithCheck(greaterIndex, nums1.length)])
                * 1.0d)
            / 2;
      } else if (greaterIndex >= nums1.length) {
        return ((nums1[smallerIndex] + nums2[remainderWithCheck(greaterIndex, nums1.length)])
                * 1.0d)
            / 2;
      } else {
