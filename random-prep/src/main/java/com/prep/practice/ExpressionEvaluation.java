...
public class ExpressionEvaluation {
  ...
  public static void main(String[] args) {
    String[][] expressions = {
      {"a = 10", "b = a++", "c = a + b", "d = c*b + a*c"}, {"z=1", "b=++z", "y=z+b"}
    };
    int[] outputs = {441, 4};
  ...
  private int evaluate(String expression, Map<Character, Integer> map) {
    ...
    for (int i = 0; i < expression.length(); i++) {
      ...
      if (ch >= 'a' && ch <= 'z') {
        ...
        if (nextIndex < expression.length()
            && secondNextIndex < expression.length()
            && (expression.charAt(secondNextIndex) >= 'a'
                && expression.charAt(secondNextIndex) <= 'z')
            && (expression.charAt(nextIndex) == '+')) {
  ...
  private int processUnaryOperation(
      Map<Character, Integer> map, String operator, Character var, boolean isPrefix) {
