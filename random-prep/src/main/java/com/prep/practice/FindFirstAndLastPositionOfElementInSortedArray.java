...
public class FindFirstAndLastPositionOfElementInSortedArray {
  ...
  public static void main(String[] args) {
    int[][] inputs = {
      {5, 7, 7, 8, 8, 8, 8, 10}, {5, 7, 7, 8, 8, 10}, {}, {2, 2}, {1, 2, 3, 3, 3, 3, 4, 5, 9}
    };
    ...
    IntStream.range(0, inputs.length)
        .forEachOrdered(
            i -> {
              System.out.println(
                  "Input : " + Arrays.toString(inputs[i]) + " target : " + targets[i]);
              System.out.println(
                  "Output : "
                      + Arrays.toString(
                          findFirstAndLastPositionOfElementInSortedArray.searchRange(
                              inputs[i], targets[i])));
