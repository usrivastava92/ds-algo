...
public class Main {
  ...
  public static void main(String[] args) {
    List<Integer> list =
        Arrays.asList(20, 23, 5, 27, 21, 16, 23, 6, 7, 24, 10, 17, 27, 9, 16, 28, 28, 16, 19, 20);
  ...
  public static int getNumDraws(int year) throws IOException {
    ...
    for (int i = 0; i <= 10; i++) {
      String uri =
          String.format(
              "https://jsonmock.hackerrank.com/api/football_matches?year=%s&team1goals=%s&team2goals=%s",
              year, i, i);
      URL url = new URL(uri);
  ...
  private static void forLoopBenchmarking() {
    ...
    System.out.println(
        "for loop insert time : " + (System.currentTimeMillis() - startTime) + " ms");
    list.clear();
