...
public class WorkflowExecutor {
  ...
  public static <RequestObject, ResponseObject> ResponseObject execute(
      AbstractWorkflow<ResponseObject> abstractWorkflow, RequestObject request) {
    ...
    for (Set<AbstractTask> taskSet : abstractWorkflow.getTaskSets()) {
      try {
        if (!taskSet.isEmpty()) {
          long maxTime =
              taskSet.stream()
                  .max(Comparator.comparingLong(AbstractTask::getTimeOfExecutionInMillis))
                  .get()
                  .getTimeOfExecutionInMillis();
          long totalTime =
              taskSet.stream().mapToLong(AbstractTask::getTimeOfExecutionInMillis).sum();
          System.out.println(
              "set will finish in : " + maxTime + " milliseconds" + " totalTime : " + totalTime);
