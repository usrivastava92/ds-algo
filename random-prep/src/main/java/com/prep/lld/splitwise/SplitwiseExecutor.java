...
public class SplitwiseExecutor {
  ...
  static {
    ...
    long minBillId = 1;
    long maxBillId = 100;
    LongStream.rangeClosed(minGroupId, maxGroupId)
        .forEach(id -> GROUP_SERVICE.createGroup(UUID.randomUUID().toString()));
    LongStream.rangeClosed(minUserId, maxUserId)
        .forEach(
            id -> {
    ...
    LongStream.rangeClosed(minBillId, maxBillId)
        .forEach(
            id -> {
              long randomGroupId = NumberUtils.getRandomLong(minGroupId, maxGroupId);
              long randomUserId = NumberUtils.getRandomLong(minUserId, maxUserId);
              while (!GROUP_SERVICE
                  .getGroup(randomGroupId)
                  .getMembers()
                  .contains(USER_SERVICE.getUser(randomUserId))) {
                randomUserId = NumberUtils.getRandomLong(minUserId, maxUserId);
              }
              double randomAmount = NumberUtils.getRandomDouble(0d, 1000d);
              Bill bill =
                  GROUP_SERVICE.addBill(
                      randomGroupId, randomAmount, "random description", randomUserId);
    ...
    for (User user : USER_SERVICE.getAllUsers()) {
      ...
      for (Map.Entry<Long, UserBalance> balanceEntry : balanceSheet.getBalanceMap().entrySet()) {
        ...
        for (Map.Entry<Long, Double> entry : balanceEntry.getValue().getSplitDetails().entrySet()) {
          System.out.println(
              "\t\t from group -> "
                  + GROUP_SERVICE.getGroup(entry.getKey())
                  + " amount -> "
                  + entry.getValue());
        }
      }
