...
public class Executor {
  ...
  public static void main(String[] args) {
    ...
    IntStream.rangeClosed(1, topicAndProducerCount)
        .forEach(
            i -> {
    ...
    IntStream.rangeClosed(1, subscriberCount)
        .forEach(
            i -> {
              String name = subscriberPrefix + i;
              Topic randomTopic =
                  broker.getTopic(topicPrefix + NumberUtils.getRandomInt(1, topicAndProducerCount));
              Subscriber subscriber = new Subscriber(name);
              System.out.println(
                  "subscriber : " + name + " subscribed to : " + randomTopic.getName());
