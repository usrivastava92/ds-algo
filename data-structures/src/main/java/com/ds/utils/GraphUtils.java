...
public class GraphUtils {
  ...
  public static <Node> Map<Node, Integer> applyDijkstra(
      IWeightedGraph<Node, Integer> graph, Node start) {
    ...
    while (!minHeap.isEmpty()) {
      ...
      for (WeightedEdge<Node, Integer> neighbourEntry :
          graph.getNeighbouringWeightedEdges(nearest)) {
  ...
  public static <Node> int applyPrims(IWeightedGraph<Node, Integer> graph) {
    ...
    PriorityQueue<WeightedEdge<Node, Integer>> minHeap =
        new PriorityQueue<>(Comparator.comparingInt(WeightedEdge::getWeight));
  ...
  public static DirectedWeightedGraph<Integer, Integer> createRandomDirectedWeightedGraph(
      int numberOfNodes) {
  ...
  public static DirectedUnweightedGraph<Integer> createRandomDirectedUnweightedGraph(
      int numberOfNodes) {
  ...
  public static UndirectedWeightedGraph<Integer, Integer> createRandomUndirectedWeightedGraph(
      int numberOfNodes) {
  ...
  public static UndirectedUnweightedGraph<Integer> createRandomUndirectedUnweightedGraph(
      int numberOfNodes) {
  ...
  public static void createRandomUnweightedEdges(
      int numberOfNodes, IUnweightedGraph<Integer> graph) {
  ...
  public static void createRandomWeightedEdges(
      int numberOfNodes, IWeightedGraph<Integer, Integer> graph) {
    for (int i = 0; i < numberOfNodes; i++) {
      for (int j = 0; j < numberOfNodes; j++) {
        if (i != j && !graph.isConnected(i, j) && NumberUtils.getRandomInt(0, 1) == 1) {
  ...
  private static <Node> boolean hasCycle(
      IUndirectedGraph<Node> graph, Node start, Node parent, Set<Node> visitedSet) {
  ...
  private static <Node> boolean hasCycle(
      IDirectedGraph<Node> graph, Node start, Map<Node, Integer> map) {
    if (!map.containsKey(start)) {
      map.put(start, 1);
      for (Node children : graph.getNeighbours(start)) {
        if ((!map.containsKey(children) && hasCycle(graph, children, map))
            || map.get(children) == 1) {
          return true;
        }
      }
  ...
  private static <Node> void topologicalHelper(
      Set<Node> visited,
      Node node,
      final IDirectedGraph<Node> graph,
  ...
  public static <Node> void union(
      Map<Node, Node> parents, Node i, Node j, Map<Node, Integer> rankMap) {
    if (parents == null || i.equals(j)) {
      return;
    }
    Node parentI = find(parents, i);
  ...
  private static <Node> void dfsForCriticalEdges(
      IUndirectedGraph<Node> graph,
      int[] timer,
      Map<Node, Integer> lows,
      Set<Node> visited,
      List<Edge<Node>> criticalEdges,
      Node node,
      Node parent) {
