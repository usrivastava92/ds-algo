...
public class SortingUtils {
  ...
  public static <T> void quickSort(List<T> list, Comparator<T> comparator, int start, int end) {
    if (!CollectionUtils.isValidIndex(list, start)
        || !CollectionUtils.isValidIndex(list, end)
        || end <= start) {
      return;
    }
    int pivotIndex = movePivotToCorrectLocation(list, comparator, end, start, end);
    System.out.printf(
        "list : %s | start : %s | end : %s | pivot : %s \n", list, start, end, pivotIndex);
    quickSort(list, comparator, start, pivotIndex - 1);
    quickSort(list, comparator, pivotIndex + 1, end);
  }

  private static <T> int movePivotToCorrectLocation(
      List<T> list, Comparator<T> comparator, int pivotIndex, int start, int end) {
    ...
    int low = start;
    int high = end;
    while (NumberUtils.isInRangeInclusive(low, start, end)
        && NumberUtils.isInRangeInclusive(high, start, end)
        && high >= low) {
      if (comparator.compare(pivot, list.get(low)) < 0
          && comparator.compare(pivot, list.get(high)) > 0) {
        swap(list, low, high);
        low++;
        high--;
      }
      if (CollectionUtils.isValidIndex(list, low)
          && comparator.compare(pivot, list.get(low)) >= 0) {
        low++;
      }
      if (CollectionUtils.isValidIndex(list, high)
          && comparator.compare(pivot, list.get(high)) <= 0) {
        high--;
      }
    }
  ...
  private static <T> void merge(
      List<T> list, Comparator<T> comparator, int low, int mid, int high) {
  ...
  public static <T> void swap(List<T> list, int index1, int index2) {
    if (!CollectionUtils.isValidIndex(list, index1)
        || !CollectionUtils.isValidIndex(list, index2)) {
      return;
    }
