...
public class TreeUtils {
  ...
  public static <T extends Comparable<T>> void insertInBST(
  ...
  public static <T extends Comparable<T>> List<List<T>> getLevelOrderTraverseListLevelWise(
      TreeNode<T> node) {
    if (node == null) {
      return Collections.emptyList();
    }
  ...
  public static <T extends Comparable<T>> List<T> levelOrderTraverseIncludingNulls(
      TreeNode<T> node) {
    if (node == null) {
      return Collections.emptyList();
    }
  ...
  private static <T extends Comparable<T>> void addNElementsInList(
      List<T> list, int times, T element) {
  ...
  public static BinaryTreeNode<Integer> createRandomBST(
      int numberOfNodes, int lowerLimitInclusive, int upperLimitInclusive) {
    if (numberOfNodes <= 0) return null;
    BinaryTreeNode<Integer> head =
        new BinaryTreeNode<>(NumberUtils.getRandomInt(lowerLimitInclusive, upperLimitInclusive));
    IntStream.generate(() -> NumberUtils.getRandomInt(lowerLimitInclusive, upperLimitInclusive))
        .limit(numberOfNodes - 1)
        .forEach(i -> insertInBST(head, i));
    return head;
  }
