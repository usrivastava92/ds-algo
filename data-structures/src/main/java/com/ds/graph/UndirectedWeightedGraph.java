...
public class UndirectedWeightedGraph<Node, Weight extends Comparable<Weight>>
    implements IWeightedGraph<Node, Weight>, IUndirectedGraph<Node> {
  ...
  public Set<Edge<Node>> getAllEdges() {
    return edges.stream()
        .map(weightedEdge -> new Edge<>(weightedEdge.getFrom(), weightedEdge.getTo()))
        .collect(Collectors.toSet());
  }
  ...
  public void deleteNode(Node node) {
    getAllEdges().stream()
        .filter(edge -> edge.getFrom().equals(node) || edge.getTo().equals(node))
        .forEach(edge -> deleteEdge(edge.getFrom(), edge.getTo()));
  }
