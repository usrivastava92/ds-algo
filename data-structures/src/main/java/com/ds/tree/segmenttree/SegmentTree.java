...
public class SegmentTree implements ISegmentTree {
  ...
  private void buildTree(int start, int end, int[] arr, int treeIndex) {
    if (!ArrayUtils.isValidIndex(arr, start)
        || !ArrayUtils.isValidIndex(arr, end)
        || !ArrayUtils.isValidIndex(tree, treeIndex)
        || end < start) {
      return;
    }
    if (start == end) {
  ...
  private int rangeHelper(final int start, final int end, int left, int right, int treeIndex) {
    ...
    return rangeHelper(start, mid, left, mid, treeIndex * 2)
        + rangeHelper(mid + 1, end, mid + 1, right, treeIndex * 2 + 1);
  }
  ...
  private void updateHelper(final int i, final int value, int left, int right, int treeIndex) {
    ...
    tree[treeIndex] =
        (ArrayUtils.isValidIndex(tree, treeIndex * 2) ? tree[treeIndex * 2] : 0)
            + (ArrayUtils.isValidIndex(tree, treeIndex * 2 + 1) ? tree[treeIndex * 2 + 1] : 0);
  }
