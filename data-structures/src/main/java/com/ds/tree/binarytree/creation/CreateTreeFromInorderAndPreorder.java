...
public class CreateTreeFromInorderAndPreorder {
  ...
  public static <T extends Comparable<T>> BinaryTreeNode<T> create(
      List<T> inorder, List<T> preorder) {
    preIndex = 0;
    return createNode(inorder, preorder, 0, preorder.size() - 1);
  }

  private static <T extends Comparable<T>> BinaryTreeNode<T> createNode(
      final List<T> inorder, final List<T> preorder, int start, int end) {
    if (start > end) {
      return null;
    }
  ...
  private static <T extends Comparable<T>> int searchItem(
      List<T> inorder, int start, int end, T item) {
