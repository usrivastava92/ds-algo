...
public class MergeInBetweenLinkedLists {
  ...
  public static void main(String[] args) {
    ListNode[] list1s = {
      LinkedListUtils.createList(0, 1, 2, 3, 4, 5), LinkedListUtils.createList(0, 1, 2, 3, 4, 5, 6)
    };
    int[] as = {3, 2};
    int[] bs = {4, 5};
    ListNode[] list2s = {
      LinkedListUtils.createList(1000000, 1000001, 1000002),
      LinkedListUtils.createList(1000000, 1000001, 1000002, 1000003, 1000004)
    };
    ListNode[] outputs = {
      LinkedListUtils.createList(0, 1, 2, 1000000, 1000001, 1000002, 5),
      LinkedListUtils.createList(0, 1, 1000000, 1000001, 1000002, 1000003, 1000004, 6)
    };
    MergeInBetweenLinkedLists mergeInBetweenLinkedLists = new MergeInBetweenLinkedLists();
    IntStream.range(0, list1s.length)
        .forEachOrdered(
            i -> {
              System.out.println(
                  "Input : list1 -> "
                      + LinkedListUtils.getTraversal(list1s[i])
                      + ", list2 -> "
                      + LinkedListUtils.getTraversal(list2s[i])
                      + ", a -> "
                      + as[i]
                      + ", b -> "
                      + bs[i]);
              ...
              Assert.assertEquals(
                  outputs[i],
                  mergeInBetweenLinkedLists.mergeInBetween(list1s[i], as[i], bs[i], list2s[i]));
            });
  }
