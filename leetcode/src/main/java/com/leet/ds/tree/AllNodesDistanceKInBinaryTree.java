...
public class AllNodesDistanceKInBinaryTree {
  ...
  public static void main(String[] args) {
    TreeNode[] roots = {
      TreeUtils.createTreeFromBfsArray(3, 5, 1, 6, 2, 0, 8, null, null, 7, 4),
      TreeUtils.createTreeFromBfsArray(0, 1, null, 3, 2),
      TreeUtils.createTreeFromBfsArray(0, 2, 1, null, null, 3)
    };
    int[] targets = {5, 2, 3};
    int[] ks = {2, 1, 3};
    ...
    IntStream.range(0, roots.length)
        .forEachOrdered(
            i -> {
              ...
              System.out.println(
                  "Output : "
                      + allNodesDistanceKInBinaryTree.distanceK(
                          roots[i], new TreeNode(targets[i]), ks[i]));
