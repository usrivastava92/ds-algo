...
public class NumbersAtMostNGivenDigitSet {
  ...
  public static void main(String[] args) {
    ...
    IntStream.range(0, digits.length)
        .forEachOrdered(
            i -> {
              System.out.println(
                  "Input : digits -> " + Arrays.toString(digits[i]) + " n -> " + n[i]);
              System.out.println(
                  "Output : " + numbersAtMostNGivenDigitSet.atMostNGivenDigitSet(digits[i], n[i]));
  ...
  public int atMostNGivenDigitSet(String[] digits, int n) {
    ...
    for (int i = 1; i <= len; i++) {
      if (i == len) {
        int numsLessOrEqualToFinalDigit =
            getNumberOfDigitsLessThanOrEqualTo(digits, comparator, targetDigits[0]);
        if (numsLessOrEqualToFinalDigit > 0) {
          boolean containsFirstDigit =
              Arrays.binarySearch(digits, targetDigits[0], comparator) >= 0;
  ...
  private <T> int getNumberOfDigitsLessThanOrEqualTo(
      T[] digits, Comparator<T> comparator, T target) {
