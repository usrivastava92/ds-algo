...
public class ValidSquare {
  ...
  public static void main(String[] args) {
    ...
    IntStream.range(0, p1s.length)
        .forEachOrdered(
            i -> {
              System.out.println(
                  "Input : ps -> "
                      + Arrays.toString(p1s[i])
                      + " p2 -> "
                      + Arrays.toString(p2s[i])
                      + " p3 -> "
                      + Arrays.toString(p3s[i])
                      + " p4 -> "
                      + Arrays.toString(p4s[i]));
              System.out.println(
                  "Output : " + validSquare.validSquare(p1s[i], p2s[i], p3s[i], p4s[i]));
