...
public class MaximizeDistanceToClosestPerson {
  ...
  public static void main(String[] args) {
    ...
    int[] outputs = {2, 3, 1, 5};
    MaximizeDistanceToClosestPerson maximizeDistanceToClosestPerson =
        new MaximizeDistanceToClosestPerson();
    IntStream.range(0, inputs.length)
        .forEachOrdered(
            i -> {
              System.out.println("Input : " + Arrays.toString(inputs[i]));
              System.out.println(
                  "Output : " + maximizeDistanceToClosestPerson.maxDistToClosest(inputs[i]));
