...
public class AddTwoNumbersII {
  ...
  public static void main(String[] args) {
    ...
    IntStream.range(0, l1s.length)
        .forEachOrdered(
            i -> {
              System.out.println(
                  "Input : l1 , "
                      + LinkedListUtils.getTraversal(l1s[i])
                      + " | l2 , "
                      + LinkedListUtils.getTraversal(l2s[i]));
              System.out.println(
                  "Output : "
                      + LinkedListUtils.getTraversal(
                          addTwoNumbersII.addTwoNumbers(l1s[i], l2s[i])));
