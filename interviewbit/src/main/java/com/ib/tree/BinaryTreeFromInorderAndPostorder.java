...
public class BinaryTreeFromInorderAndPostorder {
  public static void main(String[] args) {
    for (int i = 0; i < 1; i++) {
      ...
      BinaryTreeFromInorderAndPostorder binaryTreeFromInorderAndPostorder =
          new BinaryTreeFromInorderAndPostorder();
      TreeNode node = binaryTreeFromInorderAndPostorder.buildTree(postOrder, inOrder);
      System.out.println(
          TreeUtils.postorderTraversal(node).equals(postOrder)
              && TreeUtils.inorderTraversal(node).equals(inOrder));
    }
  }
