...
public class ReverseAlternateKNodes {
  ...
  public static void main(String[] args) {
    ListNode[] As = {
      LinkedListUtils.generateLinkedList(3, 4, 7, 5, 6, 6, 15, 61, 16),
      LinkedListUtils.generateLinkedList(1, 4, 6, 6, 4, 10)
    };
    int[] Bs = {3, 2};
    ListNode[] outputs = {
      LinkedListUtils.generateLinkedList(7, 4, 3, 5, 6, 6, 16, 61, 15),
      LinkedListUtils.generateLinkedList(4, 1, 6, 6, 10, 4)
    };
    ...
    IntStream.range(0, As.length)
        .forEachOrdered(
            i -> {
              ...
              Assert.assertEquals(
                  LinkedListUtils.toString(outputs[i]), LinkedListUtils.toString(output));
            });
  }
